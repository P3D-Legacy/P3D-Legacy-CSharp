    ...
    public static class ScriptConversion
    {
        ...
        public static double ToDouble(object expression)
        {
            ...
            else
            {
                ...
                {
                    string postFix = ToPostfix(expression.ToString(), ref ignoreError);

                    return EvaluatePostfix(postFix, ref ignoreError);
                }
                else
                    return 0d;
            }
        ...
        private static double EvaluatePostfix(string input, ref bool hasError)
        {
            ...
            while (tokens.Count > 0)
            {
                ...
                {
                    stack.Insert(0, InternalToDouble(cNumber, ref ignoreError));
                    cNumber = "";
                }
                ...
                if (IsOperator(token))
                {
                    if (stack.Count >= 2)
                    {
                        ...
                        switch (token.ToString(NumberFormatInfo.InvariantInfo))
                        {
                            case "+":
                                result = v1 + v2;
                                break;
        ...
        private static bool IsNumber(char token) => "0123456789.,".ToCharArray().Contains(token);

        /// <summary>
        /// If the token is an operator.
        /// </summary>
        /// <param name="token">The token.</param>
        private static bool IsOperator(char token) => "+-*/^%".ToCharArray().Contains(token);
        ...
        public static int ToInteger(object expression) => Convert.ToInt32(Math.Round(ToDouble(expression)));

        /// <summary>
        /// Converts an expression to a <see cref="Single"/>.
        /// </summary>
        /// <param name="expression">The expression to convert.</param>
        public static float ToSingle(object expression) => Convert.ToSingle(ToDouble(expression));
