...
namespace P3D.Legacy.Core.World
{
    ...
    public class World : IWorld
    {
        ...
        private static WeatherEnum GetRegionWeather(SeasonEnum season)
        {
        ...
        public EnvironmentTypeEnum EnvironmentType { get; set; } = EnvironmentTypeEnum.Outside;
        public WeatherEnum CurrentWeather { get; set; } = WeatherEnum.Clear;
        public bool UseLightning { get; set; } = false;
        ...
        public void Initialize(int environmentType, int weatherType)
        {
            ...
            CurrentWeather = GetWeatherFromWeatherType(weatherType);

            switch (environmentType)
            {
                case 0:
                    //Overworld
                    EnvironmentType = EnvironmentTypeEnum.Outside;
                    UseLightning = true;
                    break;
                case 1:
                    //Day always
                    EnvironmentType = EnvironmentTypeEnum.Inside;
                    UseLightning = false;
                    break;
                case 2:
                    //Cave
                    EnvironmentType = EnvironmentTypeEnum.Cave;
                    if (weatherType == 0)
                        CurrentWeather = WeatherEnum.Clear;
                    UseLightning = false;
                    break;
                case 3:
                    //Night always
                    EnvironmentType = EnvironmentTypeEnum.Dark;
                    if (weatherType == 0)
                        CurrentWeather = WeatherEnum.Clear;
                    UseLightning = false;
                    break;
                case 4:
                    //Underwater
                    EnvironmentType = EnvironmentTypeEnum.Underwater;
                    if (weatherType == 0)
                        CurrentWeather = WeatherEnum.Underwater;
                    UseLightning = true;
                    break;
                case 5:
                    //Forest
                    EnvironmentType = EnvironmentTypeEnum.Forest;
                    UseLightning = true;
                    break;
            }
            ...
            ChangeEnvironment();
            Screen.SetRenderDistance(EnvironmentType, CurrentWeather);
        }
        ...
        private void SetWeatherLevelColor()
        {
            switch (CurrentWeather)
            {
                ...
                case WeatherEnum.Snow:
                    Screen.Effect.DiffuseColor = new Vector3(0.8f);
                    break;
        ...
        private Color GetWeatherBackgroundColor(Color defaultColor)
        {
            ...
            var colorV = defaultColor.ToVector3() * Screen.SkyDome.GetWeatherColorMultiplier(v);
        ...
        public static WeatherEnum ServerWeather = WeatherEnum.Clear;
        ...
        public static int SecondsOfDay
        {
            get
            {
                if (NeedServerObject)
                {
                    ...
                    seconds += Convert.ToInt32(Math.Abs(DateDiff(DateInterval.Second, LastServerDataReceived, DateTime.Now)));
        ...
        public static int MinutesOfDay
        {
            get
            {
                if (NeedServerObject)
                {
                    ...
                    minutes += Convert.ToInt32(Math.Abs(DateDiff(DateInterval.Minute, LastServerDataReceived, DateTime.Now)));
        ...
        private static bool NeedServerObject => JoinServerScreen.Online == true && ConnectScreen.Connected == true;
