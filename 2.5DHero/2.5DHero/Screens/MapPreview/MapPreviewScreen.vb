...
Public Class MapPreviewScreen
    ...
    Private Sub UpdateHiddenItems()
        For Each e As Entity In Screens.Screen.Level.Entities
    ...
    Private Sub PickWarp()
        ...
        Dim Ray As Ray = Screens.Screen.Camera.Ray
    ...
    Private Sub ControlMap()
        ...
        If KeyBoardHandler.KeyPressed(Microsoft.Xna.Framework.Input.Keys.R) = True Or ControllerHandler.ButtonPressed(Buttons.Y) = True Then
            Core.OffsetMaps.Clear()
            Level.Load(Level.LevelFile)
        End If

        If KeyBoardHandler.KeyPressed(Microsoft.Xna.Framework.Input.Keys.Q) = True Or ControllerHandler.ButtonPressed(Buttons.X) = True Then
    ...
    Public Overrides Sub Draw()
        ...
        If Core.GameOptions.ShowGUI = True Then
            ...
            If Core.GameOptions.LoadOffsetMaps > 0 Then
                offsetString = "ON (QUALITY: " & (100 - Core.GameOptions.LoadOffsetMaps).ToString(CultureInfo.InvariantCulture) & ")"
            End If

            Dim t As String = "MAP: " & Level.LevelFile & vbNewLine &
                "LEVEL: " & Level.MapName & vbNewLine &
                "RENDERDISTANCE: " & Core.GameOptions.RenderDistance.ToString(CultureInfo.InvariantCulture) & vbNewLine &
    ...
    Private Sub DrawMapDisplays()
        For Each des As MapDisplay In Me.TextDisplays
            Dim p As Vector2 = des.Position.ProjectPoint(Screens.Screen.Camera.View, Screens.Screen.Camera.Projection)
    ...
    Public Shared Sub DetectMapPath(ByVal arg As String)
        ...
        ElseIf data.Length = 4 Then
            MapViewMode = True
            MapViewModeMapPath = data(0)
            MapViewModePosition = New Vector3(Single.Parse(data(1), NumberFormatInfo.InvariantInfo),
                                              Single.Parse(data(2), NumberFormatInfo.InvariantInfo),
                                              Single.Parse(data(3), NumberFormatInfo.InvariantInfo))
        End If
    End Sub
